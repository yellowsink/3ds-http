...
enum SYSTEM_VERSION(alias major, alias minor, alias revision) =
	(((major)<<24)|((minor)<<16)|((revision)<<8));

/// Retrieves the major version from a packed system version.
enum GET_VERSION_MAJOR(alias _version) = ((_version) >>24);

/// Retrieves the minor version from a packed system version.
enum GET_VERSION_MINOR(alias _version) = (((_version)>>16)&0xFF);

/// Retrieves the revision version from a packed system version.
enum GET_VERSION_REVISION(alias _version) = (((_version)>> 8)&0xFF);
...
/**
  ...
  */
static uint osGetMemRegionSize(MemRegion region)
{
	if(region == MemRegion.MEMREGION_ALL) {
	return osGetMemRegionSize(MemRegion.MEMREGION_APPLICATION) + osGetMemRegionSize(MemRegion.MEMREGION_SYSTEM) + osGetMemRegionSize(MemRegion.MEMREGION_BASE);
} else {
...
/**
  ...
  */
static uint osGetMemRegionUsed(MemRegion region)
{
	long mem_used;
...
/**
  ...
  */
static uint osGetMemRegionFree(MemRegion region)
{
...
/**
  ...
  */
static ubyte osGetWifiStrength()
{
...
/**
  ...
  */
static float osGet3DSliderState()
{
...
/**
  ...
  */
static bool osIsHeadsetConnected()
{
