...
enum R_SUCCEEDED(alias res) = ((res)>=0);
/// Checks whether a result code indicates failure.
enum R_FAILED(alias res) = ((res)<0);
/// Returns the level of a result code.
enum R_LEVEL(alias res) = (((res)>>27)&0x1F);
/// Returns the summary of a result code.
enum R_SUMMARY(alias res) = (((res)>>21)&0x3F);
/// Returns the module ID of a result code.
enum R_MODULE(alias res) = (((res)>>10)&0xFF);
/// Returns the description of a result code.
enum R_DESCRIPTION(alias res) = ((res)&0x3FF);

/// Builds a result code from its constituent components.
enum MAKERESULT(alias level, alias summary, alias _module, alias description) =
	((((level)&0x1F)<<27) | (((summary)&0x3F)<<21) | (((_module)&0xFF)<<10) | ((description)&0x3FF));
